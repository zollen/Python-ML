...
print("CV RMSE: %0.4f" % hb.rmse_cv(PassiveAggressiveRegressor(), train_df[all_columns], train_df['Prediction'], 5))


if False:
    test_df[['Id', 'SalePrice']].to_csv(os.path.join(PROJECT_DIR, 'data/results.csv'), index = False)
else:
    hb.write_result(os.path.join(PROJECT_DIR, 'data/models/pass_agg.csv'), train_df, test_df)
